...
const getConversationParticipants = async (conversationID) => {
  try {
    ...
    console.error(
      "Error fetching participants from users_conversations",
      error
    );
    throw error;
  }
 ...
 */
const addConversationParticipants = async (
  conversationParticipantsUserID,
  conversationID
) => {
  try {
    // const values = conversationParticipantsUserID.map((item) => `(${item}, ${conversationID})`).join(', ');
    const values = conversationParticipantsUserID
      .map(
        (item, index) => `($${2 * index + 1}::uuid, $${2 * index + 2}::integer)`
      )
      .join(", ");
    const queryText = `
...
const getUserConversations = async (userID) => {
  ...
  try {
    ...
    for (let row of result.rows) {
      ...
      const messagesData = await messageModel.getMessagesByConversation(
        row.conversation_id
      );
      const numUnreadMessages = await readReceiptsModel.getUnreadMessageCount(
        userID,
        row.conversation_id
      );
      ...
      const onlineConversationStatus = await checkConversationOnlineStatus(
        userID,
