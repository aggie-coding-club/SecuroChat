...
module.exports = function (io) {
    io.on('connection', (socket) => {
        ...
        socket.on('disconnect', async () => {
            try {
                if (socket.userId) {
                    await db.query('UPDATE users SET online_status = FALSE WHERE user_id = $1', [socket.userId]);
                    socket.broadcast.emit('userStatusChanged', { userId: socket.userId, onlineStatus: false });
                }
            } catch (error) {
                console.error('Error updating user status:', error);
            }
        ...
        socket.on('createConversation', async (conversationData) => {
            try {
                const newConversation = await ConversationModel.createConversation(conversationData);
        ...
        socket.on('addUserToConversation', async (data) => {
            try {
                await ConversationModel.addUserToConversation(data.userId, data.conversationId);
        ...
        socket.on('sendMessage', async (message) => {
            try {
                const savedMessage = await MessageModel.createMessage(message.userId, message.conversationId, message.text);
