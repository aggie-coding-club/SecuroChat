...
const sendFriendRequest = async (req, res) => {
  try {
    ...
    if (!recipientInfo.userID) {
      return res
        .status(404)
        .json({ error: "inputted username does not exist" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "successfully sent friend request" });
  } catch (error) {
...
const fetchAllCurrentFriendData = async (req, res) => {
  try {
    ...
    const friendUsernames = await friendModel.getAllCurrentUserFriends(
      decodedJWT.userID
    );
    res.status(200).json(friendUsernames);
  } catch (error) {
    console.log(
      `Error occured fetching user's fetching all of user's friend's usernames`
    );
    res.status(500).json({ error: `Internal server error.` });
  }
};
...
const rejectFriendRequest = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "successfully rejected friend request" });
  } catch (error) {
...
const updateUserOnlineStatus = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "successfully updated client online status",
      });
  } catch (error) {
