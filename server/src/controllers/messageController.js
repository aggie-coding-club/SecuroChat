...
const sendMessage = async (req, res) => {
    try {
        ...
        await messageModel.createMessage(decodedJWT.userID, messageText, conversationID);
...
const fetchMessagesByConversation = async (req, res) => {
    try {
        ...
        console.error('Error when sending a fetching conversation messages ', error);
        res.status(500).json({ error: 'Internal server error.'});
    }
};
...
const deleteMessage = async (req, res) => {
    try {
        ...
        res.status(200).json({success: true, message: 'message successfully deleted'});    
    } 
    catch (error) {
...
const updateMessage = async (req, res) => {
    try {
        ...
        res.status(200).json({success: true, message: 'message successfully updated'}); 
    } 
    catch (error) {
